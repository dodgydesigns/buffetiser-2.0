"""Add realized profit per unit to sales.
...
def upgrade() -> None:
    op.add_column(
        "sale",
        sa.Column("realized_profit_per_unit", sa.Float(), nullable=True),
    )

    if context.is_offline_mode():
        op.execute(
            "UPDATE sale SET realized_profit_per_unit = 0 "
            "WHERE realized_profit_per_unit IS NULL"
        )
        op.alter_column(
            "sale",
    ...
    for investment_key in investment_keys:
        ...
        for transaction_type, transaction in transactions:
            ...
            realized_profit_per_unit = price_per_unit - average_buy_price
            connection.execute(
                sa.text(
                    """
                    UPDATE sale
                    SET realized_profit_per_unit = :realized_profit_per_unit
                    WHERE id = :sale_id
                    """
                ),
                {
...
def downgrade() -> None:
    op.drop_column("sale", "realized_profit_per_unit")
